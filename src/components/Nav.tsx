...
function Nav() {
  ...
  return (
    <nav className=" my-4 mt-7 mx-5 relative tablet:mx-10 laptop:mx-[60px] desktop:mx-[120px]">
      <div className="flex justify-between items-center ">
        <div className="tablet:flex tablet:items-center">
          <img src="/images/logo.svg" width={121} height={33} alt="logo" />
          <ul className="tablet:flex text-grayishviolet tablet:items-center tablet:gap-x-4 tablet:ml-6 tablet:text-sm hidden">
            <li>
              <a href="#">Features</a>
            </li>
            <li>
        ...
        <div className="tablet:flex tablet:gap-x-4 tablet:items-center tablet:text-xs transition-all">
          {!menuOpen && (
            <FiMenu
              className="text-4xl transition-all tablet:hidden"
              onClick={() => handleMenu()}
            />
          )}
          {menuOpen && (
          ...
          <button className="text-grayishviolet hidden tablet:block">
            Login
          </button>
      ...
      <div
        className={clsx(
          "bg-darkgrayblue tablet:hidden transition-all text-white py-12 font-semibold px-4 text-center rounded-xl mt-[55px] absolute w-full right-0 m-auto",
          {
            "-top-[500px] ": menuOpen === false,
            "top-0": menuOpen,
          }
        )}
      >
